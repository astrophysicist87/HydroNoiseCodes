...
inline complex<double> tau_integration(complex<double> (*Gtilde_X)(double, double, double), complex<double> (*Gtilde_Y)(double, double, double), double k)
{
	...
	complex<double> Gtilde_X_pts[n_tau_pts];
	complex<double> Gtilde_Y_pts[n_tau_pts];
	for (int it = 0; it < n_tau_pts; ++it)
	{
		double t_loc = tau_pts[it];
		Gtilde_X_pts[it] = (*Gtilde_X)(k, tauf, t_loc);
		Gtilde_Y_pts[it] = (*Gtilde_Y)(-k, tauf, t_loc);
	}
...
{
	double pref = 2.0 * nu;
	complex<double> sum = tau_integration(Gtilde_rho, Gtilde_rho, k);
...
{
	double pref = 2.0 * nu;	//currently time-independent constant
	complex<double> sum = tau_integration(Gtilde_rho, Gtilde_omega, k);
...
{
	double pref = 2.0 * nu;
	complex<double> sum = tau_integration(Gtilde_omega, Gtilde_rho, k);
...
{
	double pref = 2.0 * nu;
	complex<double> sum = tau_integration(Gtilde_omega, Gtilde_omega, k);
